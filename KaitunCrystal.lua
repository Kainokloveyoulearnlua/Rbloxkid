...
getgenv().Crystal = {   
    ["Items"]= {
     ...
     ["Hop Server"] = {
        ["Hop when not run for 5 minutes"] = true,
    ...
    for _, Child in (MobTable) do
        ...
        if MonResult then
            ...
            while task.wait() do
                ...
                TweenController.Create(MobHumanoidRootPart.CFrame + Vector3.new(0,35,0))
...
FunctionsHandler.ExpRedeem:RegisterMethod("Refresh", function() 
    return ScriptStorage.PlayerData.Level < MaxLevel 
        and getsenv(game.ReplicatedStorage.GuideModule)._G.ServerData.ExpBoost == 0 
        and not Storage.Get(Storage, "IsCodesRanOut")
end)

FunctionsHandler.ExpRedeem:RegisterMethod("Start", function() 
    local Code = {
        "BANEXPLOIT","NOMOREHACK","WildDares","BossBuild","GetPranked","EARN_FRUITS",
        "Sub2UncleKizaru","FIGHT4FRUIT","kittgaming","TRIPLEABUSE","Sub2CaptainMaui",
        ...
        "CODESLIDE","fruitconcepts"
    }
    
    for Index, Promo in ipairs(Code) do
...
FunctionsHandler.LevelFarm:RegisterMethod("Start", function(Level) 
    ...
    if os.time() - LastTravel > 60 then 
        ...
        if PlayerLevel >= 1500 and SeaIndex == 2 then 
            if getgenv().Crystal["Settings"]["InSea2UntilDarkFragments"] and not ScriptStorage.Backpack["Dark Fragment"] then 
  ...
  if getgenv().Crystal["Settings"]["Farm Bones get x2Exp"] and PlayerLevel >= 2025 and (getsenv(game.ReplicatedStorage.GuideModule)._G.ServerData.ExpBoost == 0 or PlayerLevel == MaxLevel ) and ( ScriptStorage.Backpack.Bones or { Count = 0 } ).Count < 500 then 
...
FunctionsHandler.Saber:RegisterMethod("Refresh", function() 
    ...
    if not getgenv().Crystal["Items"]["Get Saber"] then 
        return 
    end 
...
FunctionsHandler.MeleesController:RegisterMethod("Start", function()
    ...
    for Cursor, Melee in MeleesTable do
        if Melee ~= 'SanguineArt' then 
            ...
            if not getgenv().Crystal["Items"]["Melee Fully Style"] then
                break 
            end 
...
FunctionsHandler.UtillyItemsActivitation:RegisterMethod("Start", function() 
    ...
    elseif Type == "Soul Reaper Spawner" then 
        ...
        if CaculateDistance((workspace.Map:FindFirstChild"Haunted Castle" or workspace:FindFirstChild"Haunted Castle"):GetModelCFrame()) > 5 then 
...
FunctionsHandler.SoulGuitar:RegisterMethod("Refresh", function() 
    ...
    if not getgenv().Crystal["Items"]["Skull Guitar"] then return end 
...
FunctionsHandler.CursedDualKatana:RegisterMethod("Refresh", function() 
    ...
    if not getgenv().Crystal["Items"]["Cursed Dual Katana"] then return end 
...
task.spawn(function()
    while task.wait(1) do
        if Window and Window.UI then
            Window.UI.Enabled = not Config.Configuration.HideUI
        end
    end
end)
...
Tabs.Home:CreateParagraph("Aligned Paragraph", {
    ...
    Content = "by Ztxgod- discord.gg/cyndral",
...
--[[SetTask("MainTask", "n/a")
    ...
    if not getgenv().Crystal["Hop Server"]["Player Nearing Hop"] then
        return
    end 
    ...
    if LastNearbyHop and os.time() - LastNearbyHop < 60 then
        return
    end
    ...
    for _, Player in ipairs(game.Players:GetPlayers()) do
        if Player ~= game.Players.LocalPlayer then
            ...
            if HRP then
                local dist = CaculateDistance(HRP.Position)
                local firstSeen = QueueList[Player.Name]

                if not firstSeen then
                    QueueList[Player.Name] = os.time()
                elseif os.time() - firstSeen > 30 then
                    if dist < 100 then
                        LastNearbyHop = os.time()
                        Hop("Nearby player: " .. Player.Name)
                        break
                    else
...
task.spawn(function()
    ...
    CoreGui.ChildAdded:Connect(function(v)
        if v.Name == "ErrorPrompt" then
            local errorText = v:FindFirstChild("MessageArea") and v.MessageArea:FindFirstChild("ErrorFrame") and v.MessageArea.ErrorFrame:FindFirstChild("ErrorMessage")
            if errorText and string.find(errorText.Text, "Disconnected") or string.find(errorText.Text, "Security kick, please rejoin") then
                task.wait(1)
...
getgenv().PlayerAdded = function(plr)
    ...
    while task.wait() do
        ...
        if getgenv().Crystal["Hop Server"]["Hop when not run for 5 minutes"] and LastIdling and os.time() - LastIdling > 60 * 5 then
